...
with tab4:
    ...
    with col1:
        ...
        age_range_import streamlit as st
import pandas as pd
...
with tab1:
    ...
    with col1:
        ...
        selected_label = st.selectbox("Timeframe", display_labels)
        ...
        qualified_only = st.checkbox("Only show qualified hitters")
        ...
        # K%
        k_filter = st.slider("K%", 0.0, 100.0, (0.0, 100.0))
        
        # BB%
        bb_filter = st.slider("BB%", 0.0, 100.0, (0.0, 100.0))
        
        # Level
        level_options = sorted(df['aLevel'].dropna().unique())
        selected_levels = st.multiselect("Level", level_options, default=level_options)
        
        # Player Name Filter
        name_query = st.text_input("Search by Player Name").strip().lower()
...
with tab2:
    ...
    with col1:
        ...
        age_range_pitch = st.slider("Age", min_age_pitch, max_age_pitch, (min_age_pitch, max_age_pitch))
        
        # IP
        min_ip = float(df_pitch['IP'].min())
        max_ip = float(df_pitch['IP'].max())
        ip_range = st.slider("Innings Pitched (IP)", min_ip, max_ip, (min_ip, max_ip))
        
        # K%
        k_filter_pitch = st.slider("K%", 0.0, 100.0, (0.0, 100.0))
        
        # BB%
        bb_filter_pitch = st.slider("BB%", 0.0, 100.0, (0.0, 100.0))
        
        # K-BB%
        kbb_min = float(df_pitch['K-BB%'].min())
        kbb_max = float(df_pitch['K-BB%'].max())
        kbb_filter = st.slider("K-BB%", kbb_min, kbb_max, (kbb_min, kbb_max))
        
        # Level
        level_options_pitch = sorted(df_pitch['aLevel'].dropna().unique())
        selected_levels_pitch = st.multiselect("Level", level_options_pitch, default=level_options_pitch)
        
        # Player Name Filter
        name_query_pitch = st.text_input("Search by Player Name").strip().lower()
...
with tab3:
    ...
    with col1:
        ...
        age_range_full_hit = st.slider("Age", min_age_full_hit, max_age_full_hit, (min_age_full_hit, max_age_full_hit))
        
        # Plate Appearances
        min_pa_full = int(df_full_hit.get("PA", pd.Series([0])).min())
        max_pa_full = int(df_full_hit.get("PA", pd.Series([600])).max())
        pa_range_full = st.slider("Plate Appearances (PA)", min_pa_full, max_pa_full, (min_pa_full, max_pa_full))
        
        # K%
        k_filter_full_hit = st.slider("K%", 0.0, 100.0, (0.0, 100.0))
        
        # BB%
        bb_filter_full_hit = st.slider("BB%", 0.0, 100.0, (0.0, 100.0))
        
        # Level
        level_options_full_hit = sorted(df_full_hit['aLevel'].dropna().unique())
        selected_levels_full_hit = st.multiselect("Level", level_options_full_hit, default=level_options_full_hit)
        
        # Player Name Filter
        name_query_full_hit = st.text_input("Search by Player Name").strip().lower()
